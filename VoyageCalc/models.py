...
class Voyage(models.Model):
    ...
    def bunkers(self):
        ...
        used_MGO = vessel.cons_bal*self.days_at_sea_ballast+vessel.cons_lad*self.days_at_sea_laden+vessel.cons_por_mgo*self.days_in_port_mgo
    ...
    def net_freight(self):
        ...
        return cost - float(self.voyage_costs())
    ...
    def over_chart(self):
        ctc_avg = self.chart_avg_tc() * 1.05
        stc = float(self.tc_equiv())
        if stc >= ctc_avg * 1.05:
            return True
        else:
            return False

    def under_chart(self):
        ctc_avg = self.chart_avg_tc() * 1.05
        stc = float(self.tc_equiv())
        if stc <= ctc_avg * 0.95:
            return True
