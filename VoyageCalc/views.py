...
def edit_voyage(request, id=None):
    ...
    title = "{} nr. {}".format('Edit Voyage', str(voyage.id).zfill(4))
    ...
    if form.is_valid():
        ...
        if finished:
            return redirect("VoyageCalc:finished-voyages")
        else:
...
def edit_income(request, id=None):
    title = 'Change Income'
    sub_btn = "Update Income"
...
def delete_income(request, id=None):
    income = Income.objects.get(id=id)
    user = request.user
    # get url of voyage-page
    came_from_page = request.GET.get('from', None)
    ...
    if next:
        ...
        return redirect(came_from_page)
...
def delete_cost(request, id=None):
    cost = Cost.objects.get(id=id)
    user = request.user
    # get url of voyage-page
    came_from_page = request.GET.get('from', None)
